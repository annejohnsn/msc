...
\begin{proof}
If $S = \{a\}$, then $\{\Gamma_S \setminus 0\} = \emptyset$ and we are done. Suppose then $S$ has at least two points and let $\mathcal{U}= \{\Gamma_S \setminus 0\}$. Let $p \in S$ be arbitrary and consider the function $\phi_p: \mathbb{R}_{\geq 0} \rightarrow \mathbb{R}$ given by $\epsilon \mapsto B(p,\epsilon) \cap \mathcal{U}$. We show $\phi_p$ is locally constant for all $p \in \mathcal{U}$.
