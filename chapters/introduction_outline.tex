...
\hl{comment from Keith:  is the integral over $\mathbb{C}$ (resp $\mathbb{C}^2$) or $supp(\mu)$? - does it matter? check measure theory notes.}\\
...
\end{definition}
\hl{comment from myself: probably should say something about when it's finite since you refer to \textit{the} equilibrium measure}\\
...
\hl{comment from Keith: explain $log^+$}
...
Chabert called this limit the valuative capacity of $E$, and we shall do the same. 
