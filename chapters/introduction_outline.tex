...
%Of course, if an element minimizes the $p-$adic valuation of product of differences in $S$, it is plain from the definition that the same element will maximize the product of $p-$adic distances in $S$, so that the above definition already bears some resemblence to the previous section. Indeed, 
%\[ v_p (\prod_{j < i} (a_i - a_j) ) = \sum_{i < j} v_p(a_i - a_j) = - log_p \rho_p(a_i - a_j)\]\\
