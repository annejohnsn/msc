...
\[v_{\gamma_k}(\sigma(n)) = \sum_{j=1}^{\alpha(k+1)-1} \lfloor \frac{n + j\cdot \beta(k)}{\alpha(k+1)\beta(k)} \rfloor\]
\end{proposition*}
...
= \sum_{j=1}^{\alpha(k+1)-1} \lfloor \frac{n + j\cdot \beta(k)}{\alpha(k+1)\beta(k)} \rfloor
\end{flalign*}
\end{proof}
...
\begin{example}
Consider the ultrametric space $(\mathbb{Z}, \rvert \cdot \lvert_p)$  for any prime $p$. Then $\beta(k)=p^k$ and $\alpha(k+1)=p$ for any $k$. The above gives 
\[v_{\gamma_k}(\sigma(n)) = \sum_{j=1}^{p-1} \lfloor \frac{n + j\cdot p^k}{p^{k+1}} \rfloor\]
and since $\gamma_k = p^{-k}$, $\forall k$, we have 
\[v_{\frac{1}{p}}(\sigma(n)) = \sum_{k=1}^{\infty} k \cdot \sum_{j=1}^{p-1} \lfloor \frac{n + j\cdot p^k}{p^{k+1}} \rfloor\]
\[ =  \sum_{k=1}^{\infty} k \cdot (\sum_{j=0}^{p-1} \lfloor \frac{n + j\cdot p^k}{p^{k+1}} \rfloor - \lfloor \frac{n}{p^{k+1}} \rfloor) \]
\[ =  \sum_{k=1}^{\infty} k \cdot (\sum_{j=0}^{p-1} \lfloor \frac{n}{p^{k+1}} + \frac{j}{p} \rfloor - \lfloor \frac{n}{p^{k+1}} \rfloor) \]
\[ =  \sum_{k=1}^{\infty} k \cdot (\lfloor p\cdot \frac{n}{p^{k+1}}\rfloor - \lfloor \frac{n}{p^{k+1}} \rfloor) \]
