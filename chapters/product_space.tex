...
\begin{proof}
Let $(M,\rho_\infty)$ be as above, and denote by $\mid \cdot \mid_{\rho_i}$ the norm on each $M_i$.Suppose also that \[\rho_i(c_ix_i,c_iy_i) = \mid c_i \mid_{\rho_i}\rho_i(x_i,y_i), \forall c_i, x_i, y_i \in M_i, \forall i.\] that is, suppose each $\rho_i$ is multiplicative.  Define a norm on $M$ by $\mid x \mid_{\rho_\infty} = \rho_{\infty}(x-0)=sup_i\{(x_i-0_i)\}=sup_i\{\mid x_i\mid_{\rho_i}\}$.  Then,  
\[
\rho_\infty(cx, cy)  
= sup_i\{\rho_i(c_i x_i, c_i y_i)\} 
= sup_i\{\mid c_i \mid_{\rho_i} \rho_i(x_i, y_i)\}
= \rho_\infty(x,y).
\] so that $\rho_\infty$ is translation invariant.  Proposition $xyz$ implies valuative capacity is as well. 
