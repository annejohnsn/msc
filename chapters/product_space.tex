...
In the next proposition, we show that scaling carries over to product space as well, although the conditions are now more restrictive. In contrast to the proposition above, here we do not allow the spaces to vary between components.
...
We now introduce two examples, whose explorations take up a large portion of the following section. 
...
We know that the capacity of $S$ will be a product of some negative power of $2$ and some negative power of $3$.  From the lemma \ref{semi-regular formula}, we know that when $\alpha(k)=2$, we have 
...
A symmetric argument shows that exponent of $3$ in the $i^{th}$ elements of the $\rho-$sequence of $S$ is 
...
\begin{conjecture} Products of $\mathbb{Z}_{p_i}$ for distinct primes have transcendental valuative capacity. \end{conjecture}
 
The aperiodicity of the sequence $\lceil\frac{i}{log_2(3)}\rceil$ over $i$ leads us to believe, but not prove, that each of the sums in $(4.1)$ and $(4.2)$ are irrational.
