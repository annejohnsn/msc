...
%\begin{example}
%Let $S$ be a compact, discretely-valued subset of $(M,\rho_N)$, an ultrametric space with metric induced by a multiplicative norm $N$. Suppose there exists $s \in S$ such that $B(0,\gamma_i) = (s^i)$ for $i \geq 0$ and $\gamma_i \in \Gamma_S$. We know from Example \ref{lagrange} that $S$ is semi-regular, but we see now that $S$ is also regular. We want to show $[s^i:s^{i+1}]$ is the same for all $i$.

%\[ B(0,\gamma^1) = \{x \in S; N(x) \leq \gamma_1\} = \{s, s^2, s^3, \ldots \}\]
%What is the distance between cosets? $\rho(B(0,\gamma_1), B(a,\gamma_1)) = \rho(0,a) = N()$
%\end{example}

%\begin{question}
%	If $S$ is a normed vector space and B=$B(0,\gamma_1)$ is a finitely-generated subgroup of $S$. Then is the set $\{c_{k} - c_{k-1}; k \in \mathbb{N}\}$ also finite? 
%\end{question}
